...
* 실행 컨텍스트는 변수나 함수같은 다른 데이터에 접근할 수 있는지, 어떻게 행동하는지를 규정합니다.
...
클로저란 다른 함수(외부함수)의 스코프에 있는 변수에 접근 가능한 함수를 지칭합니다. 함수를 값으로 사용할 수 있는 자바스크립트 언어의 특성으로, 반환된 내부함수가 다른 컨텍스트에서 실행되는 동안 내부함수의 스코프 체인에 외부함수의 스코프가 포함되어 접근이 가능하게 됩니다.
...
* 함수 선얼을 `()` 괄호로 감싸서 함수 표현식으로 바꿀수 있습니다.
